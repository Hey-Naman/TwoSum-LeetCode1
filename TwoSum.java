...
public class TwoSum{
    public static void main(String []args){
        ...
        for(int i=0;i<arraySize-1;i++){                 // outer loop to check the first Number
            for(int j=i+1;j<arraySize;j++){             // inner loop to check the second number
                if(target == array[i]+array[i+1]){
                    System.out.println("The number is found on the index "+ i+" and "+j); // if the sum is found
                    isFound = true;                     //boolean returning true if we found the number
                    return;
                }
            }
        }
